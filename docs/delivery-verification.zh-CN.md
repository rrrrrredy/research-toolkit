...
凭证绑定当前成果、简报、进度、来源与主张记录、审查日志及拟发送的交付说明；适用的需求和不确定性文件也需绑定。文本哈希将 CRLF、CR 规范为 LF。相关修改完成后再封存，后续修改会使旧凭证过期；不为重建凭证重做无关研究。
...
脚本只核对声明字段，无法发现未记录的需求、从自由文本推导阅读义务、认证引文真实性或从笔记证明理解。作者仍须对照真实请求与来源核查。独立检查器和评测器均检查需求关闭，即使关闭评测器可选的凭证检查也如此。
