...
## Review validity and independent checks

Use the [role standards](../references/subagents-and-review-loop.md) and [review-completion records](review-completion.md). Keep invocation status, substantive review validity, report verdict and final acceptance separate. Each judge covers the common rubric even when assigned a specialist emphasis. Preserve the first valid response rather than selecting the most favorable retry.

Check every review's actual coverage and evidence. Independently adjudicate critical/major findings, decisive source disputes and consequential no-change decisions. Sample accepted claims and no-finding passages as well as reported defects, using a declared population and selection method. Material discrepancies trigger checks of the affected scope; they do not automatically authorize a new full panel. An isolated reviewer error can be adjudicated without invalidating an otherwise complete review.

## Findings, repairs and stopping

In an evaluation task, retain confirmed defects as results and inputs to product iteration; do not start a repair loop merely because a sample fails. Missing mandatory material remains an unresolved finding, not a passing report. Evaluation completion does not certify report readiness.

When reader-ready report delivery or a study of repair effects is part of the task, handle that work separately. Batch justified repairs, check the changed analysis and whole report, and obtain the required four same-version reviews before accepting an edited report. Older reviews cannot approve new text. The following repair and readiness requirements apply to that report-delivery work.
...
Stop editorial iteration when these conditions hold; do not seek four empty finding lists or maximize every score. If a new substantive defect appears, reopen it. Repeated optional suggestions alone are not progress. Follow the task's declared revision opportunities and stopping rules. General permission to fix defects does not make every evaluation sample's readiness a study completion requirement. General research stagnation guidance is not a permission to abandon a mandatory requirement.
...
For each product issue retain: failing example, impact, root cause, accepted/rejected/deferred decision, change or commit, regression evidence and remaining limits. Distinguish a missing method, an existing rule not executed, a checker gap, a source-access mistake and a reviewer error. Do not automatically add all suggestions to `SKILL.md`. Revising a report after feedback while keeping the toolkit unchanged demonstrates an output repair, not a product improvement. Validate actual product changes with relevant checks; repeat or broaden them only for a new change, failure or specific unresolved concern. Unused tasks are needed for a broader effect claim, not as an automatic next round.
...
评测完成不要求每份报告修到可用。已证缺陷用于项目迭代；必读材料缺失、失败和不完整面板按冻结协议保留原状态，不能写成通过。传输或格式失败不当成质量零分，也不静默重试替换。当前面板的每个必需席位都须取得完整、有实质内容且对应版本的有效评审才算完成；调用失败或达到重试批次上限时处理原因并补齐，不能据此销账。各次失败和完成尝试单列保存，历史冻结失败不倒填为当时已完成；这不授权修改样稿，也不因已有有效评审发现问题而再次评审。
...
- 达到这些条件就停止润色，不要求模型“再也提不出意见”。发现新的实质缺陷要处理，重复的可选建议不算新进展。依任务事先确定的返工机会和停止规则执行；一般性的修复授权不把“全部评测样本可用”变成研究完成条件。

## 评审有效性、独立裁定与抽查

遵循[角色规范](../references/subagents-and-review-loop.md)和[评审完成接口](review-completion.md)，分别记录调用状态、实质评审有效性、报告结论和验收结果。各模型均覆盖共同量表，专长分工不能删掉共同维度。保留第一份有效回复，不选择更好看的重评结果。

每份评审核对实际覆盖和证据；严重/重大意见、决定性来源争议和关键不采纳决定接受独立裁定。按事先声明的方法抽查已判通过、未报问题和已报问题的内容。出现实质差错才扩大相应范围，不默认重新跑全体模型。个别误报可以逐条裁定，不因此废弃整份有效评审。
