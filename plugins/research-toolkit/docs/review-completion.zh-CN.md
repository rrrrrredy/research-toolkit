...
明确授权的报告交付任务发生变更时，追加一条 `review_plan_change`，包含 `revision_authorized: true` 以及 `previous_plan`、`current_plan` 文件引用。引用通过任务内相对路径和哈希绑定完整保留的计划。变更记录须形成连续链，末项对应当前计划；每份快照保留原作者、正文和完整席位输入绑定。共用执行器在实质任务变更且 `revision: true` 时自动生成这些记录。该标记记录调用者的授权声明，检查器不能认证真实用户同意。

退出当前计划的席位，只有匹配已核验旧计划中的任务绑定，才算合法历史；它不计入当前完成数。未知席位、被改动的快照和断裂的变更链仍会失败。原失败和负面结果必须保留。不能编造旧计划来补历史；旧版已发生的变更应从保留的原计划和真实授权恢复依据，再进行验收。冻结评测不能借此删除或改名评审席位。
...
保留所有有效性核查与证据，包括无效判定。检查器采用每次尝试的最新有效性决定，再按追加顺序选择每个席位在**当前正文和输入版本**下的第一份有效完整评审。旧尝试保留原席位编号。经过授权的报告交付计划变更可以移除或改名席位，但须按下述计划历史保留原任务绑定。
