...
1. Call `research_start` with a short task ID and the research brief assembled from the conversation. Ask the user only about the missing critical fields returned; merge their answers by calling start again while the task is in the brief stage.
